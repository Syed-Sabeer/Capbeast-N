...
@section('content')
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">eCommerce /</span> Mlb List
</h4>
...
<div class="d-flex justify-content-end mb-4">
  <a href="{{ route($prefix.'.mlb.add') }}" class="btn btn-primary">Add Mlb</a>
</div>
...
<div class="card">
  <div class="table-responsive">
    <table class="table">
      ...
      <tbody>
        @foreach($categories as $key => $mlb)
        <tr>
          <td>{{ $key + 1 }}</td>
          <td>{{ $mlb->title }}</td>
          <td>{{ Str::limit($mlb->description, 30, '...') }}</td>
          <td>
            @if($mlb->image)
              <img src="{{ asset('storage/' . $mlb->image) }}" alt="Mlb Image" width="50">
            @else
              No Image
            @endif
          </td>
          <td>
            <div class="w-25 d-flex justify-content-end">
                <label class="switch switch-primary switch-sm me-4 pe-2">
                    <input type="checkbox" class="switch-input" data-id="{{ $mlb->id }}" {{ $mlb->visibility == 1 ? 'checked' : '' }}>
          ...
          <td>
            <a href="{{ route($prefix.'.mlb.edit', $mlb->id) }}" class="btn btn-sm btn-warning">Edit</a>
            <a href="{{ route($prefix.'.mlb.delete', $mlb->id) }}" class="btn btn-sm btn-danger delete_confirm">Delete</a>
            {{-- <button type="button" class="btn btn-sm btn-danger delete-btn"
            data-url="{{ route($prefix.'.mlb.delete', $mlb->id) }}"
  ...
  document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".switch-input").forEach(function (checkbox) {
        checkbox.addEventListener("change", function () {
            ...
            const route = "{{ route($prefix . '.mlb.update.visibility', ':id') }}".replace(':id', id);
