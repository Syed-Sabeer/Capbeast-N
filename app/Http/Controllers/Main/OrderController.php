...
class OrderController extends Controller
{
  ...
  public function getStates($code)
  {
    try {
      ...
      // Update cart error with the new method
      CartHelper::trackError('shipping', $e->getMessage());
  ...
  public function PostPaymentWithPaypal($totalPrice)
  {
    try {
      ...
      // Update cart error with the new method
      CartHelper::trackError('payment', $ex->getMessage());
  ...
  public function GetPaymentStatus(Request $request)
  {
    ...
    if (empty($payerId) || empty($paymentId)) {
      // Update cart error with the new method
      CartHelper::trackError('payment', 'Missing payerId or paymentId');
    ...
    try {
      ...
      if ($result->state == 'approved') {
        ...
        if (!$checkoutDetails) {
          // Update cart error with the new method
          CartHelper::trackError('checkout', 'Session expired during payment process');
        ...
        try {
          ...
          $order = Order::create([
            ...
            'discount_type' => session('checkout_details')['discount_type'] ?? 'none',
            'discount_percentage' => session('checkout_details')['discount_percentage'] ?? 0,
          ...
          try {
            ...
            // Update cart error with the new method
            CartHelper::trackError('shipping', 'Failed to create shipment');
          ...
          // Update cart error with the new method
          CartHelper::trackError('payment', 'Order creation failed during PayPal payment process');
        ...
        CartHelper::trackError('payment', 'PayPal payment not approved');
      ...
      // Update cart error with the new method
      CartHelper::trackError('payment', 'PayPal Payment Execution Failed');
  ...
  public function index()
  {
    ...
    if (auth()->check()) {
      ...
      CartHelper::trackError('checkout', 'Checkout Page');
    }
  ...
  public function getCountries()
  {
    try {
      ...
      // Update cart error with the new method
      CartHelper::trackError('shipping', 'Error fetching countries');
    ...
    try {
      ...
      if ($request->paymentMethod === 'authorize_net') {
        ...
        if (!$paymentResult['success']) {
          // Update cart error with the new method
          CartHelper::trackError('payment', $paymentResult['message']);
        ...
        try {
          ...
          $order = Order::create([
            ...
            'subtotal_price' => $subtotalPrice,
            'discount_price' => $discountAmount,
            'discount_type' => $requestData['discount_type'] ?? 'none',
          ...
          // Update cart error with the new method
          CartHelper::trackError('payment', 'Order creation failed after Authorize.Net payment');
      ...
      // Update cart error with the new method
      CartHelper::trackError('payment', 'Checkout failed');
  ...
  public function store(Request $request)
  {
    try {
      ...
      // Update cart error with the new method
      CartHelper::trackError('payment', 'Order Placing Error: ' . $e->getMessage());
  ...
  public function processAuthorizeNetPayment($totalPrice, $cardDetails, $customerDetails)
  {
    try {
      ...
      if (empty($this->authorizeNetLoginId) || empty($this->authorizeNetTransactionKey)) {
        // Update cart error with the new method
        CartHelper::trackError('payment', 'Payment configuration error. Missing credentials');
      ...
      try {
        ...
        // Update cart error with the new method
        CartHelper::trackError('payment', $e->getMessage());
